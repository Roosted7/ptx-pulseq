...
bool ExternalSequence::load(std::string path)
{
	...
	if ( itFI != m_fileIndex.end()) {
			    ...
			    getline(data_file, buffer, MAX_LINE_SIZE)) 
		{
			...
			print_msg(DEBUG_LOW_LEVEL, std::ostringstream().flush() << "input line: " << buffer);		// ThomasR: Pulseq decrease log level
			...
			else
			{
				...
				switch (nExtensionID) {
					case EXT_LIST: 
                        ...
                        print_msg(DEBUG_MEDIUM_LEVEL, std::ostringstream().flush() << "decoding extension list entry " << buffer);		// ThomasR: Pulseq decrease log level
						m_extensionLibrary[nID] = extEntry;
						print_msg(DEBUG_HIGH_LEVEL, std::ostringstream().flush() << "nID:" << nID << " type:" << extEntry.type << " ref" << extEntry.ref << " next:" << extEntry.next);		// ThomasR: Pulseq decrease log level
						break;
					case EXT_TRIGGER: 
...
bool ExternalSequence::decodeBlock(SeqBlock *block)
{
	...
	if (block->isRF())
	{
		...
		else
		{
			...
			fDwellTime_us=m_dRadiofrequencyRasterTime_us; // ThomasR: Pulseq - allow for other RF dwell times. Default is still set to 1e-6 for old Pulseq versions
		}
		//
        // ThomasR: PTX Pulseq - START
	...
	for (int iC=GX; iC<ADC; iC++)
	{
		if (block->isArbitraryGradient(iC-GX))	// is arbitrary gradient?
		{
			...
			block->gradWaveforms[iC-GX] = std::vector<float>(waveform_gr);   // ThomasR: PTX Pulseq
